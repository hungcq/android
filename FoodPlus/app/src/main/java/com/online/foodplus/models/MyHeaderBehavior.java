...
public class MyHeaderBehavior extends CoordinatorLayout.Behavior<View> {
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, View child, View target, int dx, int dy, int[] consumed) {
        ...
        View header = coordinatorLayout.findViewById(R.id.header);
        int location[] = {0, 0};
        header.getLocationOnScreen(location);
//        View toolbar = coordinatorLayout.findViewById(R.id.toolbar);
//        Log.d("DMM", location[1] + "");
//        Log.d("DMM", toolbar.getHeight() + "");
//        Log.d("DMM", header.getHeight() + "");
//        Rect rectangle = new Rect();
//        Window window = ((MainActivity)context).getWindow();
//        window.getDecorView().getWindowVisibleDisplayFrame(rectangle);
//        int statusBarHeight = rectangle.top;
//        Log.d("DMM", statusBarHeight + "");

        if (child.getVisibility() == View.GONE
                && !mIsShowing && location[1] <= 0) {
            show(child);
        } else if(child.getVisibility() == View.VISIBLE
                && !mIsHiding && location[1] > 0) {
            hide(child);
        }
    }
